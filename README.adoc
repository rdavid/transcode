...
:img-license: https://img.shields.io/github/license/rdavid/transcode?color=8cd0d3&labelColor=555555&logo=freebsd&logoColor=dcdccc&style=flat
...
:img-rubocop: https://img.shields.io/github/actions/workflow/status/rdavid/transcode/rubocop.yml?branch=master&color=94bff3&label=rubocop&labelColor=555555&logoColor=dcdccc&style=flat
...
Transcode converts whole directories of video files in one run, from the
command line.
It wraps {url-melton}[Lisa Melton's] {url-video}[Video Transcoding], which
converts one file per invocation.
Transcode applies that same workflow to every video in a directory with a
single command.
